...
use crate::{cli::CliRunOptions, run::{run_manager::{default::DefaultRunManager, parallel::ParallelRunManager}, RunError}, task::{from_yaml::{yaml_to_json, InvalidTaskObject, YamlToJsonError}, AbstractTaskfileSource, Task, TaskInvocation, TaskRef, Workspace}};
...
pub enum TaskfileSource {
    ...
    Executable(PathBuf),
    Abstract(Arc<dyn AbstractTaskfileSource>),
}

impl TaskfileSource {
    pub fn path<'s>(&'s self) -> Cow<'s, Path> {
        match self {
            TaskfileSource::YamlFile(path) => Cow::Borrowed(path.as_path()),
            TaskfileSource::Executable(path) => Cow::Borrowed(path.as_path()),
            TaskfileSource::Abstract(source) => source.path(),
        }
    }
    ...
    //        },
    //        Self::Abstract(source) => Taskfile::from_yaml_file(source.path().as_ref())
    //            .map_err(|e| WorkspaceLoadError::Yaml(source.path().to_path_buf(), e)), // TODO wrong error
    //    }
    //}
}
